...
def get_extract_functions() -> dict[str:callable]:
...
class Drawer:
  ...
  def copy_file_with_progress(self, src, dst, progress_callback: callable):
  ...
  def _statdir(
    self,
    directory,
  ) -> tuple[list[tuple[os.DirEntry, bool, int]], int]:
    total_size = 0
    items = []
    for entry in os.scandir(directory):
      ...
      items.append(info)
      if is_dir:
        pair = self._statdir(entry)
        items.extend(pair[0])
        total_size += pair[1]
    return items, total_size

  def copy_dir_with_progress(self, src, dst, progress_callback: callable):
    """Copies given directory while providing current progress."""
    queue, total_size = self._statdir(src)
    bytes_copied = 0
    ...
    for entry, is_dir, size in queue:
      ...
      if is_dir:
        ...
        self.copy_file_with_progress(entry, entry_dst, subprogress_callback)
      bytes_copied += size
    progress_callback(bytes_copied, total_size)

  def unlink_dir_with_progress(self, directory, progress_callback: callable):
    """Deletes a given directory while providing current progress."""
    bytes_deleted = 0
    queue, total_size = self._statdir(directory)
    queue.reverse()
    for entry, is_dir, size in queue:
  ...
  def get_dir_size(self, directory) -> int:
    """Returns the size of a given directory in bytes."""
    total_size = 0
    for entry in os.scandir(directory):
      total_size += entry.stat().st_size
      if entry.is_dir():
        total_size += self.get_dir_size(entry)
    return total_size
  ...
  def is_archive_supported(self, archive) -> bool:
    ...
    supported = list(get_extract_functions().keys())
    return extension in supported

  def extract_archive(self, src, dst, progress_callback: callable = None):
    ...
    The `dst` has to be a path.
    """
    extension = filetype.guess_extension(src)
    functions = get_extract_functions()
